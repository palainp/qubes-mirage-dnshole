  ...
  let rec handle_private t trie =
    ...
    | Error `Untranslated -> (
        ...
        let forward packet =
          ...
          | Ok () -> (
              ...
              | Error `TTL_exceeded ->
                  ...
                  handle_private t trie)
        in
        let decode_dns payload =
          let dns_payload = Cstruct.to_string payload in
          match Dns.Packet.decode dns_payload with
          | Ok dns_packet when dns_packet.Dns.Packet.data = `Query -> (
              let name, _ = dns_packet.Dns.Packet.question in
              (* The packet targets a blocked address? *)
              match Dns_server.text name trie with
              | Error _ ->
                  (* We do not have a match in the blocking trie -> should forward it *)
                  Ok "forward"
              | Ok answer ->
                  (* Match the blocking trie -> propose a localhost reply *)
                  Ok answer)
          | Ok _ ->
              (* Not a Query, forward it ? *)
              Error "not_query"
          | Error e ->
              Log.err (fun m ->
                  m "couldn't decode %a: drop" Dns.Packet.pp_err e);
              Error "not_decodable"
        in
        ...
        | ip, `TCP (tcp, payload) when compare dns0 ip || compare dns1 ip -> (
            ...
            match decode_dns payload with
            (* The packet targets a blocked address? *)
            | Ok "forward" ->
                Logs.debug (fun f ->
                    f "DNS domain not found, forward to upstream");
                forward packet
            | Ok answer ->
                Logs.info (fun f -> f "DNS domain found, reply with localhost");
            ...
            | Error _ ->
                Logs.debug (fun f ->
                    f "DNS packet but not a query or not decodable: drop");
                handle_private t trie)
        | ip, `UDP (udp, payload) when compare dns0 ip || compare dns1 ip -> (
            ...
            | Ok "forward" ->
                ...
                forward packet
            | Ok answer ->
                Logs.debug (fun f -> f "DNS domain found, reply with localhost");
