...
module Underlying_arp = Arp.Make (Eth)

(* NOTE(dinosaure): [Netif] comes from [mirage-net-xen]
...
module Client_arp = struct
  ...
  let query t ip =
    if Ipaddr.V4.compare t.your_ip ip = 0 then Lwt.return_ok t.your_mac
    else Lwt.return_ok t.my_mac
end

module Client_ip = Static_ipv4.Make (Eth) (Client_arp)
...
let make backend { Dao.Client_vif.domid; _ } ~gateway ipaddr =
  let open Lwt.Syntax in
  let* ethernet = Eth.connect backend in
  let ((my_mac, your_mac) as mac) =
    (Eth.mac ethernet, Netif.frontend_mac backend)
  in
  let ((_my_ip, your_ip) as ipaddr) = (gateway, ipaddr) in
  let* arp = Client_arp.connect ~my_mac your_ip ~your_mac ethernet in
  let* ip =
    Client_ip.connect
      ~cidr:(Ipaddr.V4.Prefix.make 0 your_ip)
      ~gateway ethernet arp
  in
  Lwt.return { mac; ip; ethernet; arp; ipaddr; domid; backend }
