...
type t = {
  ...
  default_gateway : Ipaddr.V4.t;
      (* The IP that clients are given as their default gateway. *)
}

type host = [ `Client of Vif.t | `Firewall | `External of Ipaddr.t ]
...
let add_client t vif =
  ...
  let rec go () =
    ...
    | Some old ->
        ...
        Log.info (fun m ->
            m "Waiting for old client %a to go away before accepting new one"
              Vif.pp old);
...
module ARP = struct
  ...
  let input_query t arp =
    ...
    in
    if req_ipv4 = snd t.vif.Vif.ipaddr then (
  ...
  let input_gratuitous t arp =
    ...
    | Some real_mac when Macaddr.compare source_mac real_mac = 0 ->
        Log.info (fun f ->
            f "client suggests updating %s -> %s (as expected)"
              (Ipaddr.V4.to_string source_ip)
              (Macaddr.to_string source_mac))
    | Some other_mac ->
        Log.warn (fun f ->
            f "client suggests incorrect update %s -> %s (should be %s)"
              (Ipaddr.V4.to_string source_ip)
              (Macaddr.to_string source_mac)
              (Macaddr.to_string other_mac))
    | None ->
        Log.warn (fun f ->
            f "client suggests incorrect update %s -> %s (unexpected IP)"
              (Ipaddr.V4.to_string source_ip)
              (Macaddr.to_string source_mac))
